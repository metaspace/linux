...
// INVARIANTS:
//  - `self.head` points at next empty slot.
//  - `self.tail` points at last full slot except if the buffer is empty.
...
impl<T> RingBuffer<T> {
    pub fn new(capacity: usize) -> Result<Self> {
        pr_info!("Allocating RingBuffer with capacity: {capacity}\n");
        let mut this = Self {
        ...
        pr_info!("Filling ring buffer with None\n");
        for _ in 0..this.size {
            this.nodes.push_within_capacity(None)?;
        }

        pr_info!("RingBuffer allocated!\n");
...
mod tests {
    ...
    fn test_buffer_full() {
        let capacity = 3;
        let mut buffer: RingBuffer<i32> = RingBuffer::new(capacity).expect("Failed to create buffer");
