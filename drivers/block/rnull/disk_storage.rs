...
pub(crate) struct DoUnlocked<'a, 'b> {
    tree_guard: &'b mut SpinLockGuard<'a, Pin<KBox<TreeContainer>>>,
    hw_data_guard: &'b mut SpinLockGuard<'a, HwQueueContext>,
}

impl<'a, 'b> DoUnlocked<'a, 'b> {
    pub(crate) fn new(
        tree_guard: &'b mut SpinLockGuard<'a, Pin<KBox<TreeContainer>>>,
        hw_data_guard: &'b mut SpinLockGuard<'a, HwQueueContext>,
    ) -> Self {
    ...
    fn do_unlocked<U>(&mut self, cb: impl FnOnce() -> U) -> U {
        self.tree_guard
            .do_unlocked(|| self.hw_data_guard.do_unlocked(cb))
    }

    pub(crate) fn preload(&mut self) -> Result {
        let free_count = self.hw_data_guard.preload.free_count();
        if !free_count > 0 {
            let mut preload = self.do_unlocked(|| -> Result<_> {
                let mut v = KVec::new();
            ...
            self.hw_data_guard.preload.preload_with(&mut preload)?;
        }

        if self.hw_data_guard.page.is_none() {
            self.hw_data_guard.page = Some(self.do_unlocked(|| NullBlockPage::new(4096))?);
        }
